...
def get_env_type(args):
    ...
    if env_id in _engineer_envs.keys():
        env_type = env_id
        env_id = [g for g in _engineer_envs[env_type]][0]
    else:
        env_type = None
        for g, e in _engineer_envs.items():
            if env_id in e:
                env_type = g
                break
        if ':' in env_id:
            env_type = re.sub(r':.*', '', env_id)
        assert env_type is not None, 'env_id {} is not recognized in env types'.format(env_id, _engineer_envs.keys())
