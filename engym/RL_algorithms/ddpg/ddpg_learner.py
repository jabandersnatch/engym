...
from engym.RL_algorithms.ddpg.models import Actor, Critic
from engym.common.mpi_running_mean_std import RunningMeanStd
try:
...
def update_perturbed_actor(actor, perturbed_actor, param_noise_stddev):
    perturbed_actor_params = perturbed_actor.trainable_variables
    actor_params = actor.trainable_variables
    assert len(perturbed_actor_params) == len(actor_params)
    for perturbed_actor_param_, actor_param_ in zip(perturbed_actor_params, actor_params):
        perturbed_actor_param_.assign(actor_param_ + tf.random.normal(shape=actor_param_.shape, mean=0., stddev=param_noise_stddev))
